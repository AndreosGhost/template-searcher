...
public class WildcardSingleTemplateMatcher implements IMetaTemplateMatcher {
    ...
    //    Each character group is searched in the given stream for O(streamSize + groupSize) time and O
    // (groupSize) memory.
    ...
    //    If we merge two groups with sizeA and sizeB occurrences, size of their merge will be min(sizeA,
    // sizeB) and time to complete this merge is O(sizeA + sizeB) (two-pointers method).
    //    Now we can evaluate time of merging k groups into one group: O(2 * n * k/2) + O(2 * n * k/4) + ..
    // . + O(2 * n) =  O(n * k) = O(k * streamSize).
    //    In total we spend O(k * streamSize + templateSize) time and O(streamSize + templateSize) memory,
    // where k is number of continuous ? groups.
    ...
    public List<IOccurrence> matchStream(ICharStream stream) {
        ...
        if (dynamicWildcardParts.size() == dynamicPartOccurrences.size()) {
            if (dynamicWildcardParts.get(dynamicWildcardParts.size() - 1) > 0) {
                List<IOccurrence> phantomOccurrences = new LinkedList<>();
                // We insert here phantom occurrences - possible ending indices of template match (but with
                // zero length).
                for (int i = fullTemplateLength - 1, lastOccurrenceEnd = streamWrap.streamSize() - 1;
                     i <= lastOccurrenceEnd; i++) {
        ...
        while (dynamicPartOccurrences.size() > 1) {
            ArrayList<Integer> newDynamicWildCardParts = new ArrayList<>(dynamicWildcardParts.size() / 2);
            ArrayList<List<IOccurrence>> newDynamicPartOccurrences =
                    new ArrayList<>((dynamicPartOccurrences.size() + 1) / 2);
            ArrayList<Integer> newDynamicPartLengths = new ArrayList<>((dynamicPartLengths.size() + 1) / 2);
